...
void loadData(){
	...
	p[idx].data[AGE] = to_string(pd.age);
	p[idx].data[SEX] = pd.sex;
	p[idx].data[CIVIL_STATUS] = pd.cs;
	p[idx].data[ACADEMIC_GRADE] = pd.ag;
    }
}
...
void saveData(){
	...
	fout << ",";
	fout << encodeData(stoi(p[i].data[AGE]), p[i].data[SEX][0], p[i].data[CIVIL_STATUS][0], p[i].data[ACADEMIC_GRADE][0]);
    }
}
...
void assignData(person &a, vector<string> b){
    for(int i = 0; i < data_quantity; i++){
        if(b[i] == "") continue;
        a.data[i] = b[i];
    }
...
int searchDataRow(string id){
    for(int i = 0; i < (int)p.size(); i++){
        if(p[i].data[ID] == id) return i; //ID was found
    }
...
void editData(){
    int row = searchData();
    if(row == -1) return;
    assignData(p[row], readData({ID, PASSWORD}));
}
